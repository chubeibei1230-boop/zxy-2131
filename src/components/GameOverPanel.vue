...
import { Trophy, Star, RotateCcw, Target, CheckCircle2, XCircle, FileText, RefreshCw, AlertCircle, BarChart3 } from 'lucide-vue-next'
...
<template>
  <div class="glass-card p-8 text-center animate-slide-in">
    ...
    <div v-if="hasReviewReport" class="mb-3">
      <button class="btn-secondary w-full flex items-center justify-center gap-2"
              @click="emit('viewReviewReport')">
        <BarChart3 class="w-5 h-5" :style="{ color: '#FDCB6E' }" />
        <span>查看策略复盘报告</span>
      </button>
    </div>
    ...
    <div v-else class="mb-3">
      ...
      <button class="btn-secondary w-full flex items-center justify-center gap-2"
        ...
        <RefreshCw class="w-5 h-5" :class="{ 'animate-spin': isGeneratingReport }" />
        <span>{{ isGeneratingReport ? '生成中...' : '生成策略复盘报告' }}</span>
      </button>
    </div>
